...
export class EditHomePageCardsComponent extends AppComponentBase implements OnInit {
        ...
        this.addSubscription(
            combineLatest([
                ...
                .subscribe(([school, cards]) => {
                    this._school = school;
                    this.validateVirtualTour(school)
                        .pipe(
                            finalize(() => {
    ...
    /**
     * validates the school have the virtual tour page
     * @param school school info
     * @returns observable request virtual tour page
     */
    validateVirtualTour(school: ExtendedSchoolInfoResponse): Observable<boolean> {
        //If already validated, return the previous state
        if (this.validatedVirtualTour) {
            return of(this.showVirtualTour);
        }

        const virtualTourUrl =
            environment.schoolBaseSiteUrl +
            school?.scheduleTourUrl.replace('/content/gsi/us/en/', '/').replace('goddard-form.html', 'virtual-tour');

        return this._siteEditorServiceProxy.validateSchoolPageExists(virtualTourUrl);
    }
