    ...
    */
    ...
    private setConvertedEventTemplate(eventTemplate: EventTemplate) {
        ...
        if (this.eventTemplate.startTime) {
            this.startingTime = this._dateUtils.convertUTCToLocalDate(
                this._dateUtils.convertTimeSpanToDate(this.eventTemplate.startTime)
            );
        }

        if (this.eventTemplate.endTime) {
            this.endingTime = this._dateUtils.convertUTCToLocalDate(
                this._dateUtils.convertTimeSpanToDate(this.eventTemplate.endTime)
            );
        }
    ...
    private getConvertedEventTemplate(): PostEventTemplate {
        return PostEventTemplate.fromJS({
            ...this.eventTemplate,
            startTime: this.startingTime ? this._dateUtils.getTime(this.startingTime, true, false) : null,
            endTime: this.endingTime ? this._dateUtils.getTime(this.endingTime, true, false) : null,
        });
    }
        ...
        for (let index = 0; index < this.userEditableFields.length; index++) {
            ...
            if (field === 'startTime' || field === 'endTime') {
                ...
                const startTime = this.eventTemplate[field]
                    ? this._dateUtils.convertTimeSpanToDate(this.eventTemplate[field]).toISOString()
                    : undefined;
                const originalStartTime = this.originalEventTemplate[field]
                    ? this._dateUtils.convertTimeSpanToDate(this.originalEventTemplate[field]).toISOString()
                    : undefined;
